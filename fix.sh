...
# --- BLOQUE DE IMPORTACIÓN A PRUEBA DE BALAS ---
# 1. Obtenemos la carpeta donde vive este archivo (app.py)
current_dir = os.path.dirname(os.path.abspath(__file__))

# 2. Agregamos esa carpeta al 'path' de Python para que encuentre los archivos vecinos
if current_dir not in sys.path:
    sys.path.insert(0, current_dir)

# 3. Importación Híbrida
try:
    # Intento A: Modo Paquete Instalado (ej: material-lab)
    # Usa sintaxis relativa (.)
    from .database_mgr import get_all_materials, insert_from_dataframe
    from .physics import simular_ensayo
except ImportError:
    # Intento B: Modo Script Directo (ej: streamlit run app.py)
    # Como ya agregamos el 'current_dir' al path, importamos SIN PUNTOS
    from database_mgr import get_all_materials, insert_from_dataframe
    from physics import simular_ensayo
...
def render_sidebar(df_raw):
    ...
    # Validación de seguridad por si el DF viene vacío o sin columnas
...
def render_tab_management(df_mats):
    ...
    with c2:
        ...
        try:
            ...
            # Lógica para encontrar la DB en diferentes entornos
            pkg = __package__ if __package__ else 'labterial'
            # 1. Ruta de usuario (~/.labterial)
            db_path = Path.home() / f".{pkg}" / "materials.db"
            
            # 2. Si no existe, buscar relativa al script (Entorno Dev)
            if not db_path.exists():
                # src/labterial -> src -> root -> data
...
def render_tab_simulation(df_mats):
    ...
    with conf:
        ...
        lbl = "Límite (%)" if modo != "Torsion" else "Límite (rad)"
